...
class LoteAvesServiceTests {
    ...
    void adminCriaLoteComQuantidadeInicialProtegidaEEvento() {
        ...
        when(loteRepository.existsByCodigoIgnoreCase("AV-001")).thenReturn(false);
        when(instalacaoService.buscarAtiva(10L)).thenReturn(instalacao);
        ...
        verify(instalacaoService).validarCapacidade(instalacao, 50, null);
    ...
    void quantidadeAtualNaoExisteNoDtoDeEdicao() {
        assertThat(Arrays.stream(AtualizarLoteAvesRequest.class.getDeclaredFields()).map(java.lang.reflect.Field::getName))
                .doesNotContain("quantidadeAtual", "custoInicial", "status", "criadoPor", "versao");
    }
